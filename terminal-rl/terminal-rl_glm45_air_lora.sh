#!/usr/bin/env bash
# Terminal-RL GLM-4.5-Air LoRA RL tinyrun.
#
...
# to slime FSDP + PEFT LoRA because current code only supports actor-side LoRA
# adapter training under --train-backend fsdp.
#   * Use GLM-4.5-Air HF snapshot directly; no Megatron torch-dist conversion
#   * Use the lightrft_py312 conda env for Ray / sglang
#   * Tinyrun only: 1 node / 8 H200, 4 actor GPUs + 4 rollout GPUs
#   * No checkpoint save/load in validation mode
...
#
# Usage:
#   bash terminal-rl/terminal-rl_glm45_air_lora.sh
#
...
ROLLOUT_NUM_GPUS_PER_ENGINE="${ROLLOUT_NUM_GPUS_PER_ENGINE:-${ROLLOUT_GPUS_PER_NODE}}"
...
CUSTOM_CONFIG_PATH="${CUSTOM_CONFIG_PATH:-${SCRIPT_DIR}/configs/rollout_glm45_air_think.yaml}"
...
# FSDP+PEFT LoRA smoke runs skip full-model grad clipping. The FSDP actor
# treats clip_grad<=0 as disabled, avoiding LoRA/FSDP grad norm edge cases.
...
# Default: each node gives half its GPUs to actor and half to rollout.
...
if (( ACTOR_GPUS + ROLLOUT_GPUS_PER_NODE > NUM_GPUS )); then
  echo "ACTOR_GPUS(${ACTOR_GPUS}) + ROLLOUT_GPUS_PER_NODE(${ROLLOUT_GPUS_PER_NODE}) > NUM_GPUS(${NUM_GPUS})"
  exit 1
fi
log "GPU config: per_node=${NUM_GPUS}, actor_nodes=${ACTOR_NUM_NODES}, actor_per_node=${ACTOR_GPUS}, rollout_total=${ROLLOUT_GPUS}, rollout_per_node=${ROLLOUT_GPUS_PER_NODE}, engine_tp=${ROLLOUT_NUM_GPUS_PER_ENGINE}"
...
GLM45_AIR_HF_SNAPSHOT="/mnt/shared-storage-gpfs2/gpfs2-shared-public/huggingface/hub/models--zai-org--GLM-4.5-Air/snapshots/a24ceef6ce4f3536971efe9b778bdaa1bab18daa"
HF_CKPT="${HF_CKPT:-${GLM45_AIR_HF_SNAPSHOT}}"
REF_LOAD="${REF_LOAD:-${HF_CKPT}}"
if [[ ! -d "${HF_CKPT}" ]]; then
  echo "[ERROR] HF_CKPT not found: ${HF_CKPT}"
  echo "        Set HF_CKPT=/path/to/GLM-4.5-Air snapshot and rerun."
  exit 1
fi
if [[ ! -d "${REF_LOAD}" ]]; then
  echo "[ERROR] REF_LOAD not found: ${REF_LOAD}"
  echo "        For FSDP LoRA, REF_LOAD can normally point to the same HF base as HF_CKPT."
  exit 1
fi
...
if [[ "${DEBUG_MODE}" == "1" ]]; then
  RUN_NAME="${RUN_NAME:-terminal-rl_glm45-air_lora_${ACTOR_NUM_NODES}node_${NUM_GPUS}gpu_debug_${RUN_DATASET_TAG}_${RUN_ALGO_TAG}_mt${MAX_TURN}_${RUN_TIMESTAMP}}"
  # Debug mode: never save checkpoints regardless of MAX_CKPT_KEEP
  MAX_CKPT_KEEP=0
else
  RUN_NAME="${RUN_NAME:-terminal-rl_glm45-air_lora_${ACTOR_NUM_NODES}node_${NUM_GPUS}gpu_${RUN_DATASET_TAG}_${RUN_ALGO_TAG}_mt${MAX_TURN}_${RUN_TIMESTAMP}}"
fi
...
# FSDP LoRA loads the GLM HF checkpoint directly through from_pretrained().
...
ACTOR_DP_SIZE=$(( ACTOR_NUM_NODES * ACTOR_GPUS ))
...
if (( GLOBAL_BATCH_SIZE % ACTOR_DP_SIZE != 0 )); then
  echo "[ERROR] GLOBAL_BATCH_SIZE(${GLOBAL_BATCH_SIZE}) must be divisible by FSDP actor dp size ${ACTOR_DP_SIZE} (ACTOR_NUM_NODES * ACTOR_GPUS)."
  echo "[ERROR] For 1-node 4-actor-GPU tiny validation, use for example: ROLLOUT_BATCH_SIZE=4 N_SAMPLES=2 NUM_STEPS_PER_ROLLOUT=2"
  exit 1
fi
log "Batch config: rollout_batch_size=${ROLLOUT_BATCH_SIZE}, n_samples=${N_SAMPLES}, num_steps_per_rollout=${NUM_STEPS_PER_ROLLOUT}, global_batch_size=${GLOBAL_BATCH_SIZE}, actor_dp_size=${ACTOR_DP_SIZE}"
...
if [[ -n "${WANDB_KEY:-}" ]]; then
  WANDB_ARGS=(
	    ...
	    --wandb-group   "${WANDB_GROUP:-glm45-air_lora}"
	    --wandb-key     "${WANDB_KEY}"
	    --wandb-dir     "${WANDB_DIR}"
	  )
else
...
RAY_JOB_SUBMISSION_ID="${RAY_JOB_SUBMISSION_ID:-terminal_rl_glm45_air_lora_${ACTOR_NUM_NODES}node_${NUM_GPUS}gpu_$(date +%Y%m%d_%H%M%S)}"
